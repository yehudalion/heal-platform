# עד הפטור (עד הפטור) — Project State
**עודכן:** 2026-09-05 (סיום סדרת ששת סשני השבת האוטונומיים; קודם: 2026-07-10)
**שם זמני:** עד הפטור (החלטת יהודה 4.9 — `src/lib/brand.js`). הכתובת: https://highscore-eight.vercel.app/
...
## המצב ב-5.9.2026 — סיכום
| שכבה | סטטוס | הערה |
|---|---|---|
| שש פינות חיות | 🟢 | אוצר מילים, ניסוח מחדש, האזנה, השלמת משפטים, הבנת הנקרא, תחיליות וסופיות — כל אחת עם למידה/תרגול/ניתוח |
| מסך הבית + סרגלים | 🟢 | מסך בית חדש (כרטיס פתיחה, רשת שש הפינות, סימולציה), /practice במובייל, /progress עם מפת מסלול + תובנה שבועית |
| דף נחיתה | 🟢 | חדש (סשן שבת 2): הוק לאמירנט, כלים חינמיים, מספרי המאגר, סיפור המייסד (יהודה), FAQ |
| סימולציות | 🟢 | 41 טפסים (sim-1..21 מלאים, short-1..20), ציון משוער 50–150 + CEFR, דוח מלא, מסך בחירה |
| תוכן ב-DB | 🟢 | 1,369 מילים עם impact_score (823 ליבה + 546 הרחבה), 800 השלמות, 424 ניסוחים, 180 קטעי קריאה, 250 הרצאות, 84 פריטי תחיליות, 30 אתגרים יומיים אצורים |
| הגדרות לפני מנה | 🟢 | טיימר / משוב (אחרי כל שאלה או בסוף) / גודל מנה — במקום קצר/ארוך |
| PWA | 🟡 | manifest + service worker + כפתור התקנה — לבדוק בדפדפן אחרי הדחיפה |
| SEO | 🟢 | 18 כתובות ב-sitemap; 4 עמודים חדשים (ציון אמירנט, השלמה, ניסוח, קריאה) מחכים לאינדוקס |
| משחוק | 🟢 | 10 דרגות עד 9,000 XP, טוסט עם מונה, שיתוף תוצאה בכל מקום; בלי ענישה |
| מחיקת חשבון | 🟢 | Edge Function `delete-account` פרוסה, כל ה-FK ב-CASCADE |
| תשלום | 🔴 | לא קיים — רשימת המתנה במקום paywall; החלטת תמחור פתוחה |
| RLS | 🟢 | מופעל בטבלאות המשתמש |

---

# עד הפטור — Project State
**Version:** 1.0  
...
* **אומץ Master Plan v4** — מחליף את v3.1. מבוסס על ניתוח ground-truth של **116 פריטים אמיתיים
  מ-15 מבחנים רשמיים (2021–2025)**. מקור: `docs/עד הפטור_Rephrase_Master_Plan_v4_2026-07-14.md`.
